...
function fanqie(mid) {
    ...
    for (let i = 0; i < j.tracks.length; i++) {
        let temp = j.tracks[i].notes[temp.length - 1];
    ...
    function atrack(t) {
        ...
        */
        ...
        for(let i=0;i<es.length-1;i++){
            let nexti = i+1;
            for(;nexti<es.length-1;nexti++)     // 跳过“
                if(es[nexti][2][0]!="\"") break;
            switch((es[nexti][0] - es[i][0])/anote*8){
                case 1: 
            }
            i+=nexti;
        }
